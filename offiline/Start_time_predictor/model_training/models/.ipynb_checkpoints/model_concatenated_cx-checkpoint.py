...
def _training_model(train_vec, valdn_vec, ac_weights, rl_weights, 
    ...
    print("Shape of activities:", train_vec['prefixes']['activities'].shape)
    print("Shape of roles:", train_vec['prefixes']['roles'].shape)
    print("Shape of next_evt_act:", train_vec['prefixes']['next_evt_act'].shape)
    print("Shape of times:", train_vec['prefixes']['times'].shape)
    print("Shape of inter_attr:", train_vec['prefixes']['inter_attr'].shape)

    print("Shape of next_evt activities:", train_vec['next_evt']['activities'].shape)
    print("Shape of next_evt roles:", train_vec['next_evt']['roles'].shape)
    print("Shape of next_evt times:", train_vec['next_evt']['times'].shape)
...
# =============================================================================
    ...
    next_event = Input(shape=(train_vec['prefixes']['next_evt_act'].shape[1],), name='next_event')

    t_input = Input(shape=(train_vec['prefixes']['times'].shape[1],
                           train_vec['prefixes']['times'].shape[2]), name='t_input')

    # t_input = Input(shape=(train_vec['prefixes']['times'].shape[1],1), name='t_input')

    inter_input = Input(shape=(train_vec['prefixes']['inter_attr'].shape[1],
                            train_vec['prefixes']['inter_attr'].shape[2]),
                     name='inter_input')

    print("Shape of ac_input:", ac_input.shape)
    print("Shape of rl_input:", rl_input.shape)
    print("Shape of next_event:", next_event.shape)
    print("Shape of t_input:", t_input.shape)
    print("Shape of inter_input:", inter_input.shape)
...
# =============================================================================
    ...
    print("Shape of ac_embedding:", ac_embedding.shape)
    print("Shape of rl_embedding:", rl_embedding.shape)


    next_event_reshaped = Reshape((train_vec['prefixes']['next_evt_act'].shape[1], 1))(next_event)

    # =============================================================================
#    Layer 1
# =============================================================================
    concatenate = Concatenate(name='concatenated', axis=2)(
        [ac_embedding, rl_embedding,next_event_reshaped, t_input, inter_input])
...
# =============================================================================
    l2_c1 = Bidirectional(LSTM(args['l_size'],
                 kernel_initializer='glorot_uniform',
...
#   The layer specialized in role prediction
    l2_c2 = Bidirectional(LSTM(args['l_size'],
                 kernel_initializer='glorot_uniform',
                 return_sequences=False,
                 dropout=0.2,
                 implementation=args['imp']))(batch1)
...
#   The layer specialized in role prediction
    l2_c3 = Bidirectional(LSTM(args['l_size'],
                activation=args['lstm_act'],
                kernel_initializer='glorot_uniform',
                return_sequences=False,
                dropout=0.2,
...
#                         name='role_output')(l2_c2)
    ...
    if ('dense_act' in args) and (args['dense_act'] is not None):
        ...
        time_output = Dense(train_vec['next_evt']['times'].shape[1],
                            kernel_initializer='glorot_uniform',
                            name='time_output')(l2_c3)
    model = Model(inputs=[ac_input, rl_input, next_event, t_input, inter_input],
                  outputs=[time_output])
    ...
    model.fit({'ac_input': train_vec['prefixes']['activities'],
               ...
               'next_event': train_vec['prefixes']['next_evt_act'],
              ...
              validation_data=(
                  {'ac_input': valdn_vec['prefixes']['activities'],
                   ...
                   'next_event': valdn_vec['prefixes']['next_evt_act'],
