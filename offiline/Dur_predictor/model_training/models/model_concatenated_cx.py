...
def _training_model(train_vec, valdn_vec, ac_weights, rl_weights, 
    ...
    print("Shape of next_evt_act:", train_vec['prefixes']['next_evt_start'].shape)
...
# =============================================================================
    ...
    rl_input = Input(shape=(train_vec['prefixes']['roles'].shape[1], ),
                     name='rl_input')
    next_event_start = Input(shape=(train_vec['prefixes']['next_evt_start'].shape[1],), name='next_event_start')
...
#    Layer 1
# =============================================================================
    next_event_reshaped_act = Reshape((train_vec['prefixes']['next_evt_act'].shape[1], 1))(next_event)

    next_event_reshaped = Reshape((train_vec['prefixes']['next_evt_start'].shape[1], 1))(next_event_start)
    concatenate = Concatenate(name='concatenated', axis=2)(
        [ac_embedding, rl_embedding,next_event_reshaped_act,next_event_reshaped, t_input, inter_input])
...
# =============================================================================
    ...
    if ('dense_act' in args) and (args['dense_act'] is not None):
        ...
        time_output = Dense(train_vec['next_evt']['times'].shape[1],
                            kernel_initializer='glorot_uniform',
                            name='time_output')(l2_c3)
    model = Model(inputs=[ac_input, rl_input,next_event,next_event_start, t_input, inter_input],
                  outputs=[ time_output])
    ...
    model.fit({'ac_input': train_vec['prefixes']['activities'],
               ...
               'next_event_start': train_vec['prefixes']['next_evt_start'],
              ...
              validation_data=(
                  {'ac_input': valdn_vec['prefixes']['activities'],
                   ...
                   'next_event_start': valdn_vec['prefixes']['next_evt_start'],
