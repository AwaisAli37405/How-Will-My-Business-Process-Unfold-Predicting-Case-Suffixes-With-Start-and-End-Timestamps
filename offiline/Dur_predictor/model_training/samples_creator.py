...
class SequencesCreator():
    ...
    def times_vector(self,log,parms):
        self.log=log
        ...
        durations = [entry['dur'] for entry in self.log]
        waitings = [entry['wait'] for entry in self.log]
    ...
    def _vectorize_seq_inter(self, parms, columns):
        ...
        vec['prefixes']['next_evt_act'] = vec['next_evt']['activities']

        # Determine the target sequence length based on 'activities' in prefixes
        target_length = vec['prefixes']['activities'].shape[1]
        target = np.zeros((vec['next_evt']['activities'].size, target_length), dtype=int)
        target[:, -1] = vec['prefixes']['next_evt_act']

        vec['prefixes']['next_evt_act'] = target
        ...
        # Determine the target sequence length based on 'activities' in prefixes
        target_length = vec['prefixes']['activities'].shape[1]
        target = np.zeros((np.array(y_times_dict['wait_norm']).size, target_length), dtype=float)
        wait_norm = np.array(y_times_dict['wait_norm'])
        target[:, -1] = wait_norm


        vec['prefixes']['next_evt_start'] = target



        vec['next_evt']['times'] = np.dstack(list(y_times_dict['dur_norm']))[0].reshape(-1, 1) # next dur output
