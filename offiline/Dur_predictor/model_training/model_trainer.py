...
class ModelTrainer():
    ...
    def __init__(self, params):
        ...
        # self.save_optimizer(output_path,optimizer)
        # Remove folder
        shutil.rmtree(params['output'])
        
    # def save_optimizer(self, output_folder,optimizer):
    #     output_directory = os.path.join(output_folder, 'optimizer')
    #     if not os.path.exists(output_directory):
    #         os.makedirs(output_directory)
    #
    #     # Save the model
    #     filename = os.path.join(output_directory, 'model_hmm.pkl')
    #     with open(filename, "wb") as file:
    #         pickle.dump(optimizer.optimizer, file)
    #
    #     # Save the scaler
    #     filename = os.path.join(output_directory, 'scaler.pkl')
    #     with open(filename, "wb") as file:
    #         pickle.dump(optimizer.scaler, file)
    ...
    def preprocess(self, params):
        ...
        # add the active cases and the available resources to the log
        self.log = feat.get_resource_availability(self.log, params, self.resource_counts)
    ...
    def load_embedded(index, filename):
        ...
        with open(os.path.join(input_folder, filename), 'r') as csvfile:
            ...
            for row in filereader:
                cat_ix = int(row[0])
                if str(index[cat_ix]).strip() == row[1].strip():
