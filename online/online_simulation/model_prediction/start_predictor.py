...
class StartPredictor():
    ...
    def __init__(self, parms):
        self.output_route_act_pred = os.path.join('output_files', parms['start_predictor'])
    ...
    def predict(self, log, model):
        ...
        for event in reformatted_events:
            ...
            vec = self.create_samples(trace, col=['wait_norm', 'active_cases_len', 'available_resources','inter_arrival_time',
                                                  'ac_index', 'rl_index', 'dur_norm'])  # trace.iloc[0].to_frame().T

            # now predicting the duration
            print(f"Predicting start for case {case}")
            pred = self.predict_case(vec, model)
            # now update the activity instance

            # print(f"Trace before update: {trace.iloc[-1]}")
            trace_index = trace.index[-1]
            log.loc[trace_index, 'wait_norm'] = pred[0][0]


            rescaled_wait = rescale(pred[0][0], self.parms, self.parms['scale_args']['wait'])
            # Convert rescaled_wait to timedelta for consistent handling
            rescaled_wait = rescaled_wait

            # Convert max wait in log to timedelta for comparison
            max_wait_timedelta = np.max(log['wait'])

            # Ensure rescaled_wait is at least 1 second
            if rescaled_wait < 0:
                rescaled_wait = 1

            # Ensure rescaled_wait does not exceed the maximum wait in the log
            if rescaled_wait > max_wait_timedelta:
                rescaled_wait = max_wait_timedelta
            # rescaled_wait = min(rescaled_wait, np.max(log['wait']))

            log.loc[trace_index, 'wait'] = rescaled_wait
            if len(trace.index) > 1:  # Check if there's at least one previous row
                log.loc[trace_index, 'start_timestamp'] = log.loc[trace.index[-2], 'start_timestamp'] + timedelta(
                    seconds=rescaled_wait.total_seconds() if isinstance(rescaled_wait, timedelta) else rescaled_wait
                )
            else:
                log.loc[trace_index, 'start_timestamp'] = timedelta(
                    seconds=rescaled_wait.total_seconds() if isinstance(rescaled_wait, timedelta) else rescaled_wait
                )

            # print(f"Trace after update: {log.loc[trace_index]}")
    ...
    def predict_case(self, prefix, model):
        ...
        inputs = [x_ac_ngram, x_rl_ngram, next_event, x_t_ngram, x_inter_ngram]
